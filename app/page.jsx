...
import { pages, blogPosts, pathFor } from '../lib/routes'
...
export default function Page() {
  return <main id="top" className="min-h-screen overflow-hidden bg-[var(--paper)] text-[var(--ink)]">
    ...
    <section id="offices" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28"><SectionIntro eyebrow="Where we are" title="Local presence. National reach." copy="Contact the relevant city team for advice on your matter." /><div className="grid gap-5 md:grid-cols-3">{cities.map(city => <div key={city.id} className="rounded-2xl border border-black/10 p-6"><MapPin className="mb-8 text-[var(--brass)]" size={21} /><h3 className="font-serif text-2xl">{city.name}</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{city.description}</p><a className="mt-7 inline-block text-sm font-semibold" href={`tel:${city.phone.replaceAll(' ', '')}`}>{city.phone} <ArrowUpRight className="ml-1 inline" size={14} /></a></div>)}</div></section>

    <section id="insights" className="border-t border-black/10 bg-[#eeeae2] px-5 py-20 lg:px-8 lg:py-28"><div className="mx-auto max-w-7xl"><SectionIntro eyebrow="From our desk" title="Useful perspective on the law." /><div className="grid gap-6 md:grid-cols-3">{blogPosts.map(article => <article key={article.slug} className="group"><Link href={pathFor(article.slug)} className="mb-6 block aspect-[1.45] rounded-2xl bg-[var(--ink)] p-6 text-white"><span className="text-xs uppercase tracking-[0.18em] text-[var(--brass)]">Taxation</span><div className="flex h-full items-end"><p className="font-serif text-2xl leading-tight">{article.title}</p></div></Link><p className="text-xs text-[var(--muted)]">{formatDate(article.date)}</p><h3 className="mt-2 text-lg font-semibold leading-snug group-hover:text-[var(--brass)]">{article.summary}</h3></article>)}</div></div></section>
